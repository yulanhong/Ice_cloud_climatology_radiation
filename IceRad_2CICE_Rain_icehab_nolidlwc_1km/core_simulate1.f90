...
!                print *,values
        ...
        !==============ice part================================
                ...
                !============inquire ice layers=========================
                                ice_lay_1=&
                             sum(Ice_Rain_Info)/count(Ice_Rain_Info>0.0)
!                                print *,ice_lay_1,Ice_Rain_Info,&
!                                        count(Ice_Rain_Info>0.0)
                                IF(abs(ice_lay_1-ice_lay_2)>0.96 .and.&
                                        ice_lay_2 /=0.0)then
                                        ice_water_lay=ice_water_lay+1
                                ENDIF
                                        ice_lay_2=ice_lay_1
                ...
                !===========inquire water layer=========================
                   IF (lwc_s1(i) > 0.0) THEN

                          water_flag_d=water_flag_d+1
                          water_lay_1=hgt_s(i)
                          water_mark(i)=hgt_s(i)

                          IF (abs(water_lay_1-water_lay_2)>0.96 &
                              .and. water_lay_2 /=0) then
                               ice_water_lay=ice_water_lay+1
       ...
       ! average DARDAR
                ...
                DO i=0,3
                        IF (dj <= 418 .AND. iwc_s1(dj-i) > 0 .AND. &
                         iwc_s1(dj-i) <= 10.0 &
               ...
               ! when layer begins
                ...
                ! too many layers        
                IF (ice_flag >=55 .and. dj>k1+4) THEN
                   write(200,"(x,f6.3,2x,f12.6,2x,f6.2)") &
                        mean_hgt_1,mean_iwc_1,mean_rei_1
                        k1=dj

                ELSE IF (ice_flag <55) then
...
!                IF (water_flag_d > 0) THEN
                ...
                !============merge laysers=======================
                        if ((ice_rain_flag+water_flag_d)>=70) then
                                wk=3
                        else
                                wk=2
                        endif
                ...
                IF ((ice_flag+water_flag_d+ice_water_lay+&
                        rain_flag+2)>=59 .and. hi>k2+wk) THEN
                        write(300,"(x,f6.3,2x,f12.6,2x,f6.2)") &

               sum(water_mark(hi:hi+wk))/count(water_mark(hi:hi+wk)>0),&
                sum(lwc_s1(hi:hi+wk))/count(water_mark(hi:hi+wk)>0),&
                sum(rew_s1(hi:hi+wk))/count(water_mark(hi:hi+wk)>0)

!                        water_lay_1= &
!                sum(water_mark(hi-1:hi))/count(water_mark(hi-1:hi)>0)
                        k2=hi
                        
                        water_flag_s = water_flag_s+1
                        ELSE IF &
                        ((ice_flag+water_flag_d+ice_water_lay+&
                        rain_flag+2)< 59) then
                ...
                IF (water_flag_d == 0 .and. no_rain>0) then
                   ...
                   IF ((59-ice_rain_flag-ice_water_lay-2)&
                        < 0) then ! if too many
! water layer, reduce the resolution
                    rain_hgt=mean_hgt_1!for rain in all alt.
!
!                    
                    mean_hgt_1=int(mean_hgt_1)   

                    if (rain_top ==0 ) then
                        water_lay_2= rain_hgt+0.48
                        write (300,"(x,f6.3,2x,f12.6,2x,f6.2)"),&
                        mean_hgt_1+0.12,0.0,0.0
                    endif
                        rain_top=rain_top+1
...
!                ENDIF !end write water profile
        ...
        !=================== atmospheric conditions===================
                ...
                IF((pres(hi,si)*pres(hi+1,si)) < 0 .and. (hi+1) <= 125)&
                 then 
...
!                print *,temp_lat,temp_lon
                ...
                IF (noon_sza< 88) THEN  ! when the sun presents
...
!               print *, temp_lat,temp_lon,vis_abd,noon_sza
                OPEN (500,file="swinput.inp")
                        ...
                        IF (icloudy /= 3 ) THEN
                        !totally clear
                              IF (water_flag_s > 0 .or. rain_flag_s >0)&
                                then
                                write(500,*) "wc_layer"
...
!                 print *, file_size
                ...
                pres_swa(:,si)=-99.0
                temp_swa(:,si)=-99.0
                FD_swa(:,si,icloudy)=-99.0
                FU_swa(:,si,icloudy)=-99.0
                heat_swa(:,si,icloudy)=-99.0
...
!                        write(400,*) "nstr 6"
                        ...
                        IF (icloudy /= 3) then
                        IF (water_flag_s > 0 .or. rain_flag_s > 0)then
                                write(400,*) "wc_layer"
                ...
                IF (file_size == 0 .or. file_size == -1) goto 2000              
...
!                print *, fd_out 
  2000         ENDDO ! end clear and cloudy
!                stop
